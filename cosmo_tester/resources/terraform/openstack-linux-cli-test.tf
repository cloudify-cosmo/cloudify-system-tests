...
resource "openstack_compute_instance_v2" "cli_server" {
  ...
  provisioner "file" {
    source = "scripts/linux-cli-test.sh"
    destination = "/tmp/linux-cli-test.sh"

  }

  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/linux-cli-test.sh",
      "/tmp/linux-cli-test.sh ${var.cli_package_url} /tmp/key.pem ${openstack_networking_floatingip_v2.manager_floatingip.address} ${openstack_compute_instance_v2.manager_server.network.0.fixed_ip_v4} ${var.manager_user}"
    ]
  }
}
