...
export PATH="/Users/admin/Library/Python/2.7/bin:/usr/local/bin:$PATH"
...
cfy profiles use ${PUBLIC_IP} -u admin -p admin -t default_tenant

cfy blueprints upload cloudify-cosmo/cloudify-hello-world-example -b bp -n singlehost-blueprint.yaml
cfy deployments create -b bp dep -i server_ip=${PUBLIC_IP} -i agent_user=${MANAGER_USER} -i agent_private_key_path=${AGENT_KEY_PATH}
...
curl http://${PUBLIC_IP}:8080 2>&1 | grep "Hello, World"
