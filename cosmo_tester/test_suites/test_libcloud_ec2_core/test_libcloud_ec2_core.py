...
CLOUDIFY_TEST_CONFIG_PATH = 'd:/projects/github/cloudify-libcloud-provider/cloudify_libcloud/cloudify-config.yaml'
CLOUDIFY_TEST_DEFAULTS_CONFIG_PATH = 'd:/projects/github/cloudify-libcloud-provider/cloudify_libcloud/cloudify-config.defaults.yaml'
...
class LibcloudEC2ProviderTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.provider_config = self._read_config(CLOUDIFY_TEST_CONFIG_PATH, CLOUDIFY_TEST_DEFAULTS_CONFIG_PATH)
        self._update_config(self.name_prefix, self.provider_config)
        self.provider_manager = common.ProviderManager(provider_config=self.provider_config)
    ...
    def tearDown(self):
        super(LibcloudEC2ProviderTest, self).tearDown()
    ...
    def _validate_key_pairs(self, compute_config):
        created = self._get_key_pair_list_names()
        self.assertIn(
    ...
    def _validate_management_server(self, management_config, public_ip):
        ...
        self.assertEqual(
            ...
            ' but created {0}: {1}'.format(created_len, ', '.join(created_names)))
        node = created[0]
        ...
        if 'floating_ip' in management_config:
            self._validate_publick_ip(management_config['floating_ip'], public_ip)
    ...
    def _validate_teardowned(self):
        ...
        self.assertEqual(
            ...
            ' during teardown process: ' + ', '.join(created))
        created, created_names = self._get_node_list_w_names(NodeState.TERMINATED)
        self.assertEqual(
            len(created_names),
            0,
    ...
    def _get_key_pair_list_names(self):
        created = []
    ...
    def _get_node_list_w_names(self, state=None):
        created = []
        created_names = []
