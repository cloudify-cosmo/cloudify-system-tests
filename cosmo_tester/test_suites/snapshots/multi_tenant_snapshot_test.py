...
def test_restore_snapshot_and_agents_upgrade_multitenant(
    ...
    make_tenants(old_manager, logger, tenants=new_tenants)
    ...
    for tenant in noinstall_tenants:
        create_noinstall_helloworld(old_manager, logger, tenant=tenant)

    create_tenant_secrets(old_manager, tenants, logger)

    old_plugins = {
        tenant: upload_test_plugin(old_manager, logger, tenant)
...
def make_tenants(manager, logger, tenants=('tenant1', 'tenant2')):
    for tenant in tenants:
