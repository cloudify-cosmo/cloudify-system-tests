...
pre_bootstrap_files = None


def _get_dir_structure(mgr):
...
def preconfigure_callback(managers):
    global pre_bootstrap_files
    mgr = managers[0]
    pre_bootstrap_files = _get_dir_structure(mgr)

    # Some manual additions, as we know these files will be generated by the BS
    pre_bootstrap_files['yum packages'] += [
        'python-pip', 'libxslt', 'daemonize'
    ]
    pre_bootstrap_files['folders in /opt'] += ['python_NOTICE.txt', 'lib']
    pre_bootstrap_files['folders in /var/log'] += ['yum.log']
    pre_bootstrap_files['init_d service files (/etc/rc.d/init.d/)'] += [
...
def manager(request, cfy, ssh_key, module_tmpdir, attributes, logger):
    ...
    cluster = CloudifyCluster.create_bootstrap_based(
            ...
            preconfigure_callback=preconfigure_callback)
...
def test_teardown(cfy, manager):
    cfy.teardown('-f')
    current_files = _get_dir_structure(manager)
    diffs = {}

    for key in current_files:
        pre_bootstrap_set = set(pre_bootstrap_files[key])
        current_set = set(current_files[key])

        diff = current_set - pre_bootstrap_set
        if diff:
            diffs[key] = diff

    assert not diffs, 'The following files were not removed: ' \
                      '{0}'.format(diffs)
