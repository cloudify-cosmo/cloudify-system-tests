...
def test_tmux_session_linux(linux_cli_tester, logger):
    cli_host = linux_cli_tester['cli_host']
    example = linux_cli_tester['example']
    paths = linux_cli_tester['paths']

    _prepare(cli_host.run_command, example, paths, logger)
    _test_tmux_session(cli_host.run_command,
...
def test_cfy_logs_linux_cluster(linux_cluster_cli_tester, logger):
    ...
    for node in nodes:
        node.run_command('cfy_manager stop')
    ...
    for node in nodes:
        log_hashes = [f.split()[0] for f in node.run_command(
            ...
            use_sudo=True
        ).stdout.splitlines() if not ('access' in f or 'patroni' in f)]
        node_dump_filepaths = \
        ...
        for dump_filepath in node_dump_filepaths:
            ...
            mutable_files = ['cloudify/journalctl.log',
                             'cloudify/nginx/cloudify.access.log',
                             'cloudify/db_cluster/patroni/patroni.log']
            for f in mutable_files:
                assert local_dump_path / f in files
            log_hashes_local = sorted(
                [hashlib.md5(open(f, 'rb').read()).hexdigest() for f in
                 files if not ('journalctl' in f.name or
                               'access' in f.name or
                               'patroni' in f.name or
                               'startup_check' in f.name)])
