...
def test_teardown(bootstrap_test_manager, ssh_key, logger, test_config):
    ...
    bootstrapped_state = get_system_state(bootstrap_test_manager)
    expected_diffs = {}
    ...
    current_state = get_system_state(bootstrap_test_manager)
    diffs = {}
...
def check_pre_bootstrap_state(manager):
    ...
    pre_bootstrap_state = get_system_state(manager)
...
def get_system_state(mgr, hide_stdout=False):
    with mgr.ssh() as fabric:
        hide = 'stdout' if hide_stdout else None
        systemd = fabric.run('ls /usr/lib/systemd/system',
                             hide=hide).stdout.split()
        init_d = fabric.run('ls /etc/rc.d/init.d/', hide=hide).stdout.split()
        sysconfig = fabric.run('ls /etc/sysconfig', hide=hide).stdout.split()
        opt_dirs = fabric.run('ls /opt', hide=hide).stdout.split()
        etc_dirs = fabric.run('ls /etc', hide=hide).stdout.split()

        packages = fabric.run('rpm -qa', hide=hide).stdout.split()
        # Prettify the packages output
        packages = [package.rsplit('-', 2)[0] for package in packages]

        users = fabric.run('cut -d: -f1 /etc/passwd', hide=hide).stdout.split()
        groups = fabric.run('cut -d: -f1 /etc/group', hide=hide).stdout.split()
    return {
